...
-- =====================================================
-- FLOATING BUTTON (DELTA COMPATIBLE)
-- Arrastra para mover · Click para abrir/cerrar
-- =====================================================
task.spawn(function()
    local CoreGui   = game:GetService("CoreGui")
    ...
    pcall(function()
        local old = CoreGui:FindFirstChild("JoshHubToggleBtn")
        if old then old:Destroy() end
    end)

    local sg = Instance.new("ScreenGui")
    sg.Name            = "JoshHubToggleBtn"
    ...
    btn.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            -- Si no hubo drag (menos de 5px) es un click
            if dragging then
                local moved = (inp.Position - dragStart).Magnitude
                if moved < 5 then
                    -- Toggle UI
                    Library:Toggle()
                    local isOpen = Library.Toggled
                    dot.BackgroundColor3 = isOpen
                        and Color3.fromRGB(80, 255, 120)
                        or  Color3.fromRGB(255, 80, 80)
                    TweenSvc:Create(btn, TweenInfo.new(0.1), {
                        BackgroundColor3 = isOpen
                            and Color3.fromRGB(18, 18, 26)
                            or  Color3.fromRGB(30, 18, 18)
                    }):Play()
                end
            end
            dragging = false
    ...
    -- Sync dot with UI state
    task.spawn(function()
        while sg and sg.Parent do
            dot.BackgroundColor3 = Library.Toggled
                and Color3.fromRGB(80, 255, 120)
                or  Color3.fromRGB(255, 80, 80)
            task.wait(0.5)
        end
